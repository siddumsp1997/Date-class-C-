...
DateFormat(const char* dateFormat, const char* monthFormat, const char* yearFormat)/*throw(formatexception)*/;

// Single C-string format where dateFormat, monthFormat, and yearFormat are separated by '-'
// Example: "dd-mmm-yy", "d-mm-yyyy", etc
DateFormat(const char* format) /*throw(formatexception)*/;

// Default DateFormat created as "dd-mmm-yy"
DateFormat();

//My additional functions and operators
DateFormat& operator=(const DateFormat& x);//copy assignment operator
char* getdate(); //getting the date
char* getmonth(); //getting the month
char* getyear(); //getting the year
...
//throw(invalid_argument, domain_error, out_of_range,formatexception);
...
Date operator+(int noOfDays); // Day noOfDays after the current date
Date operator-(int noOfDays); // Day noOfDays before the current date
//throw(domain_error, out_of_range);
